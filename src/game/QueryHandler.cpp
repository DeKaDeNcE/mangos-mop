...
void WorldSession::HandleNameQueryOpcode(WorldPacket& recv_data)
{
    ...
    switch (bytesLeft)
    {
        case 4:
        {
            ...
            if (realmID != realmId)
            {
                sLog.outError("Warning! Quering players from other realms is not supported. Queried realm id: %u This realm id: %u", realmId, realmID);
...
void WorldSession::HandleRealmQueryOpcode(WorldPacket& recv_data)
{
    ...
    if (realmId == realmID)
        ...
        name = sWorld.GetRealmName(realmID);
